...
import requests
from tsms.tsms_decorator import use_logging, params_check, Logit,logit
import tsms.tsms_config
...
class Tsmstest(object):
    def __init__(self, env=0):
        ...
        self.exp_str = ''
        # self.exp_list = []
        self.head_url = tsms.tsms_config.header_url
        # self.user = tsms.tsms_config.user
        # self.passwd = tsms.tsms_config.passwd
        # 初始化一个全局session对象
        # self.s = requests.session()
        self.status_code = ''
        self.text = ''
        self.json = {}
        self.root_user = tsms.tsms_config.root_user
        self.root_passwd = tsms.tsms_config.root_passwd
    ...
    def tsms_get(self, url_type, user, passwd,*, headers=None):
    ...
    @logit
    def req_post(self, url_type, data, user = 'carl', passwd = 'lijihua198915', *, headers=None):
        '''post请求方法封装'''
        url = self.choie_url(url_type)
        if headers:
            hd = headers
    ...
    @logit
    def req_put(self, url_type, data, user = 'carl', passwd = 'lijihua198915',*, headers=None):
        '''put请求方法封装'''
        url = self.choie_url(url_type)
        if headers:
            hd = headers
    ...
    @logit
    def req_del(self, url_type, data, user = 'carl', passwd = 'lijihua198915', *, headers=None):
        '''del请求方法封装'''
        url = self.choie_url(url_type)
        if headers:
            hd = headers
    ...
    @logit
    def req_audit(self, audit_type, reviwe_id, audit_status, reject_reason = None):
        '''审核功能接口，兼容签名和模版审核'''
        data = {
