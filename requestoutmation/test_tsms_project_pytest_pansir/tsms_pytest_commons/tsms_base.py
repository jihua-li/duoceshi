...
import requests
from common_tools.common_decorator import logging, logit
...
class TsmsBase(object):
    ...
    def choice_url(self, url_type):
        # if url_type == "dd":
        #     return
        if url_config.get(url_type):
            # http://127.0.0.1:5001  +  /v1/signature
            return self.head_url + url_config.get(url_type)
        else:
            return url_type
    ...
    def sign_data(self, del_key=None, none_key=None, signature=None, source=None, pics=None):
        signature = signature if signature else self.gen_ranstr(3, 3)
        # if signature == "random":
        #     signature = self.gen_ranstr(3, 3)
        # elif signature == None:
        #     signature == None
        # if signature:
        #     signature = signature
    ...
    def ljh_data(self, flag, **kwargs):
        ...
        for k, v in kwargs:
            if k in data_config.get("ljh_filed"):
                data[k] = v
        return data
