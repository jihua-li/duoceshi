import logging, os, random, string, requests, json, pytest
from pytest_commons.tsms_decorator import use_logging, logit
from pytest_config import tsms_base_config
...
class Tsmstest(object):
    def __init__(self, env=None):
        ...
        if self.env == 0:
            ...
            logging.info('head_url: {}'.format(self.head_url))
        else:
            self.head_url = tsms_base_config.header_url
            self.user = tsms_base_config.user
            self.passwd = tsms_base_config.passwd
        ...
        self.phone_heads = tsms_base_config.data_config.get('default_heads')
        self.sign_id = tsms_base_config.data_config.get('default_sign_id')
        self.temp_id = tsms_base_config.data_config.get('default_temp_id')
    ...
    @logit
    def req_del(self, url_type, data, user=None, passwd=None, *, headers=None):
        '''del请求方法封装'''
        url = self.choie_url(url_type)
        if headers:
            hd = headers
    ...
    def sign_data(self, signature=None, source=None, pics=None):
        '''sign接口请求数据'''
        signature = signature if signature else self.gen_ranstr(1, 4)
        source = source if source else self.gen_ranstr(0, 6)
    ...
    @logit
    def send_data(self, num=1, sign_id=None, temp_id=None):
        sign_id = sign_id if sign_id else self.sign_id
        temp_id = temp_id if temp_id else self.temp_id
        data = {
            "sign_id": sign_id,  # 必填
            "temp_id": temp_id,  # 必填
    ...
    def gen_para_fail(self, para_type):
        ...
        def para_fail(request):
            ...
            测试夹具，测试所有异常data
            '''
            # logging.info("[param is]: {}".format(request.param))
