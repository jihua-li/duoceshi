import logging,os,random,string,requests
from tsms_project.commons.tsms_decorator import use_logging, logit
from tsms_project.config import tsms_base_config

logging.basicConfig(level=logging.INFO, format='%(asctime)16s - %(levelname)8s - %(message)s')


class Tsmstest(object):
    def __init__(self, env=2):
        self.headers = {
        ...
        self.exp_str = ''
        # self.exp_list = []
        logging.info('当前读取的env为：{}'.format(env))
        if env:
            self.env = env
        else:
